...
> 主仓路径：`D:\AIContentFactory\三方闭环整合项目\` · 旧副本已归档：`D:\三方闭环整合项目_archived_20260821\`（Phase 0 改名归档，可恢复）

> **本清单定位**：T001–T016 是**方法论脚手架**（搭建任务系统本身，已于 v0.2–v0.7 完成），属样例 / 元任务，并非业务执行队列。
> **真实业务任务清单（实时）**在 `D:\AIContentFactory\tasks\backlog.md`（T001–T335+，书类内容工厂执行队列），不在本模板仓内；本清单不重复登记，避免双份漂移。规范驱动业务的方式 = 业务仓按 `tasks/README.md`、`docs/risk-approval.md`、`PR_CHECKLIST.md` 运行，本模板仅提供可复用的方法论。
